...
> **多操作系统支持：** 本实验支持在 **Ubuntu** 和 **麒麟操作系统（Kylin OS）** 上完成。两者的主要差异在于包管理器不同（Ubuntu 使用 `apt`，麒麟OS 使用 `yum`），以及部分软件包名称有所区别，教程中会同时给出两种系统的安装命令。
>
> **多架构支持：** 本实验以 x86（IA-32/x86_64）为目标架构，使用 NASM 汇编器和 QEMU 模拟器。
> - **x86_64 环境**（Ubuntu 或麒麟OS）：所有实验内容均可直接完成。使用 `qemu-system-i386` 或 `qemu-system-x86_64` 启动 MBR 程序。
> - **ARM64 环境**（麒麟OS ARM64 等）：NASM 是跨平台汇编器，可在 ARM64 上编译 x86 二进制代码；QEMU 通过 TCG（Tiny Code Generator）软件翻译模式在 ARM64 上模拟 x86 处理器。因此 Assignment 1、2、4 可以正常完成。Assignment 3（32位 ELF 链接）在 ARM64 上需要额外配置，详见对应章节说明。

# 实验环境配置

## 安装基础工具

本实验需要安装 NASM 汇编器、QEMU 模拟器和 GDB 调试器。
...
> **说明：** 
> - NASM（Netwide Assembler）是一款跨平台的 x86 汇编器，无论宿主机是 x86_64 还是 ARM64，都能将 x86 汇编代码编译为 x86 二进制文件。
...
NASM（Netwide Assembler）是 Intel x86 汇编代码的编译器，支持 Linux、macOS、Windows 等多种平台。由于 NASM 本质上是一个**交叉汇编器**（cross-assembler），即使在 ARM64 架构的计算机上运行，也能正确编译出 x86 架构的二进制文件。

> 同学们看到的一些汇编书籍如王爽《汇编语言》，《Intel汇编语言程序设计》使用的是MASM汇编。MASM是在MS-DOS下运行的。而NASM汇编是跨平台的，且二者语法有部分不同。但无论是NASM还是MASM，都属于x86汇编（或称为Intel汇编）的内容，关于汇编指令如mov，add，jmp等常用指令是相同的。因此，如果下面的内容和nasm无关，教程会标注为x86汇编；否则教程会标注为nasm汇编。
...
> **麒麟OS用户提示：** 如果你的系统上没有 `qemu-system-i386`（如某些麒麟OS版本），可以使用 `qemu-system-x86_64` 代替：
> ```shell
> qemu-system-x86_64 -hda hd.img -serial null -parallel stdio
> ```
> `qemu-system-x86_64` 完全兼容16位实模式代码，运行效果与 `qemu-system-i386` 一致。
...
因为同学们可能是首次编写汇编代码，所以在过程中可能会遇到一些困难，请灵活使用debug工具（见下）进行调试；也可以提前阅读一下[注意事项](./README.md#注意事项)，避免踩助教曾经踩过的雷。
...
ARM64 上的 `ld` 不支持 `-melf_i386` 参数。如果你在实验一中已构建了 i686-elf 交叉编译工具链，可以使用：
...
> 如果没有交叉工具链，也可以跳过生成符号表的步骤，直接使用 GDB 连接 QEMU 进行基于地址的调试（通过 `x/10i $pc` 查看反汇编指令，`info registers` 查看寄存器等）。
...
> **麒麟OS用户提示：** 若没有 `qemu-system-i386`，使用 `qemu-system-x86_64` 代替即可，GDB 远程调试功能完全一致。
...
> **ARM64 环境提示：** ARM64 上的 GDB 默认支持的是 AArch64 架构。连接 QEMU 远程调试 x86 程序时，GDB 会自动识别远程目标的架构，基本的调试功能（如断点、单步、查看内存和寄存器）通常可以正常使用。如果遇到架构不匹配的问题，可尝试在 GDB 中手动设置：
> ```
...
同学们可以从[gdb教程](http://c.biancheng.net/view/8123.html)中学习到gdb的常用调试命令，但对于本次实验来说，用到的命令大多数是下面几个。
...
## 注意事项
汇总一些助教踩过的雷，**仅供参考**，也欢迎大家投稿。
1. 在使用标号的时候，如果放在代码中间，有可能会遇到一些问题（比如使用gdb调试时，单步运行到标号定义行时，可能会跳过标号后面的多行代码。所以，请考虑将你的标号定义到最后，比如：
```assembly
end2:
jmp $ ; 死循环
...
2. 在进行gdb debug的时候，在生成符号表时，我们需要先删掉`mbr.asm`开头的`org`伪指令，但是在将`mbr.asm`载入到硬盘时，还是需要保留这行伪指令。所以建议先保留`org`伪指令，生成二进制文件，载入`hd.img`；随后再删去`org`伪指令，生成符号表，并开始gdb debug。

3. 在进行gdb debug的时候，如果单步运行到`int 10h`这种中断指令时，会导致代码无法继续调试。这时你可以使用断点+continue的方式，跳过这个中断指令。比如：
```assembly
11    mov ah, 0x02
12    mov bh, 0x00
13    int 0x10
14    mov ah, 0x09  ; INT 10h, 显示字符
```
你可以在gdb中直接break 14，即在第14行代码打上断点。类似地，你还可以break 某个代码标号，比如`break end2`，然后continue即可直接执行到断点处。

4. **麒麟OS 用户注意事项：**
   - 如果你的系统没有 `qemu-system-i386`，在所有命令中将其替换为 `qemu-system-x86_64` 即可。
   - ARM64 架构的麒麟OS上，`ld -melf_i386` 不可用，生成符号表时请使用实验一中构建的 `i686-elf-ld` 工具，或者直接使用基于地址的调试方式。
   - QEMU 在 ARM64 上运行 x86 代码使用软件翻译（TCG模式），速度较 x86_64 原生运行会慢一些，属于正常现象。
...
17. 汇编代码的编写
    ...
    > + 寄存器请使用32位的寄存器。
    > + **Ubuntu：** 执行命令 `sudo apt install gcc-multilib g++-multilib` 安装相应环境。
    > + **麒麟OS（x86_64）：** 执行命令 `sudo yum install gcc-c++ glibc-devel.i686 libstdc++-devel.i686` 安装相应环境。
    > + **麒麟OS（ARM64）：** ARM64 架构上无法直接使用 `-m32` 编译32位 x86 程序。请参考 Assignment 3 中的 ARM64 说明。
...
> 以下记录了在麒麟操作系统（Kylin Linux Advanced Server V11，ARM64架构）上完成本实验各步骤的验证过程，供同学们参考。
...
在麒麟OS上编译和运行 MBR 程序的步骤与 Ubuntu 基本相同，唯一区别是使用 `qemu-system-x86_64` 代替 `qemu-system-i386`：
...
# 4. 启动 QEMU（使用 qemu-system-x86_64）
qemu-system-x86_64 -hda hd.img -serial null -parallel stdio
```

NASM 是跨平台的 x86 汇编器，即使在 ARM64 主机上也能正确编译 x86 汇编代码。QEMU 通过 TCG（Tiny Code Generator）在 ARM64 上软件模拟 x86 处理器，因此所有基于 QEMU 的 MBR 实验均可正常完成。